#include <EEPROM.h>
#include <string.h>
#include <Wire.h>
#include <SoftwareSerial.h>


#define in1 13        //left_input1
#define in2 9         //left_input1
#define EN1 11        //left_enable1
#define TRIG_PIN1 6   //left_trig
#define ECHO_PIN1 5   //left_echo 
//-----------------------------------------


#define in3 2         //right_input2
#define in4 4         //right_input2
#define EN2 11        //right_enable2
#define TRIG_PIN2 10  //right_trig
#define ECHO_PIN2 3   //right_echo
//-----------------------------------------

#define red 12       //red_RGB_led
#define blue 8       //blue_RGB_led
#define green 7      //green_RGB_led
//-----------------------------------------


//*****************************************
// Variable Declaration
//*****************************************
long duration1;   //duration for left sensor
long duration2;  //duration for right sensor
String Mode;     //variable to select mode
int button;      // variable to control direction manually
String speed;    // variable takes speed as string from user
int sp;          //variable controls speed level
int safe_distance; // fixed distance from wall 



void setup() {
  Serial.begin(9600);  //baud rate

  //define the inputs and outputs 

  pinMode(in1,OUTPUT) ;
  pinMode(in2,OUTPUT) ;
  pinMode(in3,OUTPUT) ;
  pinMode(in4,OUTPUT) ;
  pinMode(EN1,OUTPUT) ;
  pinMode(EN2,OUTPUT) ;
  pinMode(TRIG_PIN1,OUTPUT);
  pinMode(ECHO_PIN1,INPUT);
  pinMode(TRIG_PIN2,OUTPUT);
  pinMode(ECHO_PIN2,INPUT);


}

void loop() {
  Serial.println("Choose Car Speed");

  while (!Serial.available()) {
    // Wait for user input
  }
  if(Serial.available())
  { 
    speed=Serial.readString();
    Serial.println(speed);
    speed.toUpperCase();
    if (speed.equals("LOW"))
    {
      sp = 25;
      digitalWrite(green, LOW);
      digitalWrite(blue, LOW);
      digitalWrite(red, HIGH);
      delay(1000);
    }
    else if (speed.equals("MEDIUM"))
    {
      sp = 127;
      digitalWrite(green, LOW);
      digitalWrite(red, LOW);
      digitalWrite(blue, HIGH);
      delay(1000);
    }
    else if (speed.equals("HIGH"))
    {
      sp = 255;
      digitalWrite(red, LOW);
      digitalWrite(blue, LOW);
      digitalWrite(green, HIGH);
      delay(1000);

    } 
  }
  Serial.println("Choose Car Mode");
  while (!Serial.available()) {
    // Wait for user input
  }
  if(Serial.available())
  {
    Mode=Serial.readString();
    Serial.println(Mode);
    Mode.toUpperCase();

    if(Mode.equals("MANUAL"))
    {
      Manual();
    }

    else if(Mode.equals("AUTO"))
    {
      while(1)
      {
          Autonomous();
      }
    } 
  }  

}

//****************************************
// define of functions 
//****************************************

//function make car move forward

void Forward()
{
  analogWrite(EN1,sp);   
  analogWrite(EN2,sp);
  digitalWrite(in1,HIGH);   //left wheel moves forward
  digitalWrite(in2,LOW);
  digitalWrite(in3,HIGH);   // Right wheel moves forward
  digitalWrite(in4,LOW);  
  delay(1000); //1 second

}

//function make car move backward

void Backward()
{
  analogWrite(EN1,sp);   
  analogWrite(EN2,sp);
  digitalWrite(in1,LOW);   //left wheel moves Backward
  digitalWrite(in2,HIGH);
  digitalWrite(in3,LOW);   // Right wheel moves Backward
  digitalWrite(in4,HIGH);  
  delay(1000);   //1 second

}

//function make car move right

void Right()
{
  analogWrite(EN1,sp);   
  analogWrite(EN2,sp);
  digitalWrite(in1,LOW);   //left wheel moves Backward
  digitalWrite(in2,HIGH);
  digitalWrite(in3,HIGH);   //Right wheel moves Forward
  digitalWrite(in4,LOW);  
  delay(1000);   //1 second
 
}

//function make car move left

void Left()
{
  analogWrite(EN1,sp);   
  analogWrite(EN2,sp);
  digitalWrite(in1,HIGH);   //left wheel moves Forward
  digitalWrite(in2,LOW);
  digitalWrite(in3,LOW);   // Right wheel moves backward
  digitalWrite(in4,HIGH);  
  delay(1000);  //1 second
 
}

//function make car stop

void Stop()
{
  analogWrite(EN1,0);   // when sending to ENABLE 0 it stops moving 
  analogWrite(EN2,0);
  digitalWrite(in1,LOW);  
  digitalWrite(in2,LOW);
  digitalWrite(in3,LOW);   
  digitalWrite(in4,LOW);
  delay(200);   //0.5 second
}

//function make car to move manual

void Manual()
{
  Serial.println("Choose Car Position : 1:forward ,2:backward,3:Right,4:left ");
  while (!Serial.available()) {
    // Wait for user input
  }
  if(Serial.available())
  {
    String b;        // button as string
    b=Serial.readString();
    button=b.toInt();
    switch (button)
    {
        case 1: // forward 
        {      
          Forward();
          break;
        }
        case 2:        //backward
        {      
          Backward();
          break;
        }   
        case 3:        //Right
        {      
          Right();
          break;
        }
        case 4:        //left           
        {      
          Left();
          break;
        }       
    }  
  } 
}  

// function define the role of ultrasonic
int repeat = 0;
void Autonomous () {
  if(repeat == 0)
  {
    String d;
    Serial.println("Enter safe distance:");
    
    while (!Serial.available()) {
      // Wait for user input
    }
    
    if (Serial.available()) {
      d = Serial.readString();
      Serial.println(d);
      safe_distance = d.toInt();
    }
  }
  repeat++;

  // Left ultrasonic sensor
  digitalWrite(TRIG_PIN1, LOW);
  delayMicroseconds(2);
  digitalWrite(TRIG_PIN1, HIGH);
  delayMicroseconds(10);
  digitalWrite(TRIG_PIN1, LOW);
  duration1 = pulseIn(ECHO_PIN1, HIGH);
  float distance_left = duration1 * 0.034 / 2;

  // Right ultrasonic sensor
  digitalWrite(TRIG_PIN2, LOW);
  delayMicroseconds(2);
  digitalWrite(TRIG_PIN2, HIGH);
  delayMicroseconds(10);
  digitalWrite(TRIG_PIN2, LOW);
  duration2 = pulseIn(ECHO_PIN2, HIGH);
  float distance_right = duration2 * 0.034 / 2;

  // Print the distance at the right side
  Serial.print("Distance_right: ");
  Serial.print(distance_right);
  Serial.println(" cm");
  delay(100);

  // Print the distance at the left side
  Serial.print("Distance_left: ");
  Serial.print(distance_left);
  Serial.println(" cm");
  delay(100);

  Forward(); // Start moving forward

  // Move left if the car is too close to the right wall
  if (distance_right < safe_distance) {
    Stop();
    while (distance_right < safe_distance) {
      Left();
      Stop();
      // Recheck the right distance
      digitalWrite(TRIG_PIN2, LOW);
      delayMicroseconds(2);
      digitalWrite(TRIG_PIN2, HIGH);
      delayMicroseconds(10);
      digitalWrite(TRIG_PIN2, LOW);
      duration2 = pulseIn(ECHO_PIN2, HIGH);
      distance_right = duration2 * 0.034 / 2;
    }
    delay(10);
  }

  // Move right if the car is too close to the left wall
  else if (distance_left < safe_distance) {
    Stop();
    while (distance_left < safe_distance) {
      Right();
      Stop();
      // Recheck the left distance
      digitalWrite(TRIG_PIN1, LOW);
      delayMicroseconds(2);
      digitalWrite(TRIG_PIN1, HIGH);
      delayMicroseconds(10);
      digitalWrite(TRIG_PIN1, LOW);
      duration1 = pulseIn(ECHO_PIN1, HIGH);
      distance_left = duration1 * 0.034 / 2;
    }
    delay(10);
  }
}

